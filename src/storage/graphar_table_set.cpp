...
void GraphArTableSet::CreateTables(GraphArCatalog &graphar_catalog, const InfoVector &infos, GraphArTableType type) {
	...
	for (auto &info: infos) {
		...
		for (idx_t i = 0; i < bind_data->GetFlattenPropNames().size(); ++i) {
			columns.emplace_back(bind_data->GetFlattenPropNames()[i], GraphArFunctions::graphArT2duckT(bind_data->GetFlattenPropTypes()[i]));
		}
		...
		auto table_info = make_shared_ptr<GraphArTableInformation>(graphar_catalog, std::move(table_entry), file_name, type, bind_data->GetParams());
