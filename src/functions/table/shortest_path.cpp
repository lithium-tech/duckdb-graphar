...
unique_ptr<GlobalTableFunctionState> ShortestPath::InitGlobal(ClientContext& context,
    ...
    auto forward_edges_result = graphar::EdgesCollection::Make(
        bind_data.graph_info,
        bind_data.edge_info->GetSrcType(),
        bind_data.edge_info->GetEdgeType(),
        bind_data.edge_info->GetDstType(),
        graphar::AdjListType::ordered_by_source
    );
    
    if (forward_edges_result.has_error()) {
        throw InvalidInputException("Failed to create forward edges collection for: " + 
                                   bind_data.edge_info->GetSrcType() + "--" +
                                   bind_data.edge_info->GetEdgeType() + "->" +
                                   bind_data.edge_info->GetDstType());
    }
    ...
    if (backward_edges_result.has_error()) {
        throw InvalidInputException("Failed to create backward edges collection for: " + 
                                   bind_data.edge_info->GetSrcType() + "--" +
                                   bind_data.edge_info->GetEdgeType() + "->" +
                                   bind_data.edge_info->GetDstType());
    }
    ...
    if (bind_data.start_id == bind_data.end_id) {
        ...
        while (!q_forward.empty() && !q_backward.empty() && !found) {
            ...
            for (idx_t i = 0; i < backward_level_size && !found; i++) {
                ...
                auto backward_iter = global_state->backward_edges->find_dst(curr, global_state->backward_edges->begin());
...
TableFunction ShortestPath::GetFunction() {
    TableFunction function("shortest_path", 
...
void ShortestPath::Register(ExtensionLoader& loader) {
