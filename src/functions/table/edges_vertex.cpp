...
// State Init
//-------------------------------------------------------------------
unique_ptr<GlobalTableFunctionState> EdgesVertexGlobalTableFunctionState::Init(ClientContext &context, TableFunctionInitInput &input) {
	bool time_logging = GraphArSettings::is_time_logging(context);
	...
	return make_uniq<EdgesVertexGlobalTableFunctionState>(context, bind_data, bind_data.GetEdgeInfo(), prefix, iter, end_iter);
}
//-------------------------------------------------------------------
...
graphar::Result<vector<std::pair<graphar::IdType, graphar::IdType>>> EdgesVertex::GetAdjListOffsetOfVertices(
	...
	for (idx_t i = 0; i < chunks_num; ++i) {
		for (idx_t j = 0; j < arrays[i]->length(); ++j) {
			...
			auto np = std::make_pair(static_cast<graphar::IdType>(arrays[i]->Value(j)),
			                         static_cast<graphar::IdType>(nxt));
...
inline void EdgesVertex::Execute(ClientContext &context, TableFunctionInput &input, DataChunk &output) {
	...
	vector<std::pair<graphar::IdType, graphar::IdType>> result = GetAdjListOffsetOfVertices(context, gstate.GetEdgeInfo(), gstate.GetPrefix(), graphar::AdjListType::ordered_by_source, start, end).value();
...
void EdgesVertex::Register(DatabaseInstance &db) {
