...
public:
    ...
    void OpenTable(graphar::IdType vid) {
        ...
        if (offset_chunk_index != current_offset_chunk_index) {
            ...
            auto maybe_table = fs->ReadFileToTable(path, file_type);
            if (maybe_table.has_error()) {
                throw InternalException("Error converting reader: " + maybe_table.error().message());
            }

            current_table = maybe_table.value();
            current_offset_chunk_index = offset_chunk_index;
            DUCKDB_GRAPHAR_LOG_DEBUG("Offset table opened, chunk index: " + std::to_string(current_offset_chunk_index));
        } else {
            DUCKDB_GRAPHAR_LOG_DEBUG("equal offset chunk index " + std::to_string(offset_chunk_index));
        }
    }
